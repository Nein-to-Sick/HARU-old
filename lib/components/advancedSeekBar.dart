...
class SeekBarRenderBox extends RenderBox {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (_seekBarDragState == SeekBarDragState.FINISH) {
      context.canvas.drawCircle(
          Offset(progress * size.width, size.height / 2 - 40),
          scaleWhileDrag ? thumbSize / 2 * 2 : thumbSize / 2,
          Paint()
            ..color = thumbColor
            ..style = PaintingStyle.fill);
      context.canvas.drawPath(
        _createInvertedTrianglePath(
            progress * size.width, size.height / 2 - 30, thumbSize * 2),
      ...
      final textOffset = Offset(
        progress * size.width - textPainter.width / 2,
        size.height / 2 - 30 - textPainter.height / 2 - 8,
      );
  ...
  Path _createInvertedTrianglePath(double centerX, double centerY, int size) {
    final path = Path();
    final halfSize = size / 2;

    path.moveTo(centerX - halfSize + 1.6,
        centerY + halfSize - 17.9); // Bottom-left corner
    path.lineTo(centerX + halfSize - 1.6,
        centerY + halfSize - 17.9); // Bottom-right corner
    path.lineTo(centerX, centerY + halfSize * 2 - 12); // Top corner
    path.close();
